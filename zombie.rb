    ...
    def enter()
      ...
      def fight()
        ...
        while (@zombie.health > 0)
          ...
          ## If player decide to rest, health is improved
          if choice == "rest"
            @player.rest()
            puts "You rested and now have #{@player.health} health points."
          ## If player decided to fight, attack the zombie
          elsif choice == "fight"
        ...
        ## Zombie's turns to attack!
        @zombie.attack(@player)

        ## If the zombie attacked killed player, return 'dead'
        if (@player.health <= 0)
